...
- **THEN** it SHALL display a rail of parcelles with per-parcelle status (at minimum: worker count, pending gates, unread/alert count, overseer state)
...
- **THEN** it SHALL be delivered to the general lane, not to any parcelle overseer
...
- **THEN** the system SHALL create that parcelle and route its subsequent events to a (new) overseer for it

### Requirement: Control-Room tmux Topology

Overseers SHALL be presented as tmux windows within the conductor session on the existing per-vignoble tmux server; workers SHALL remain flat tmux sessions on the same server.

#### Scenario: One window per parcelle plus dashboard
- **WHEN** the control room is running with active parcelles A and B
- **THEN** the conductor session SHALL have a dashboard window AND one overseer window per active parcelle
- **AND** no additional tmux server SHALL be created per parcelle

#### Scenario: Switch attaches to the real overseer session
- **WHEN** the human selects a parcelle from the dashboard (or switches tmux window)
- **THEN** tmux SHALL switch to that parcelle overseer's own full Pi TUI window (no mirroring)
...
- **AND** listing the conductor session's windows SHALL show the parcelle overseers
